...
const excludedKupaNamesB = ['קיימות', '50-60','קצבה','עד 50','לבני 50',
    'לגילאי 50','50 עד','עד 60','60 ומעלה','50 ומטה'
];
const excludedOchlosiyaB = ['עובדי סקטור מסויים', 'עובדי מפעל/גוף מסויים'];
const excludedMozarB = ['מטרה אחרת'];
const excludedMasB = ['מבטיח תשואה'];
const mozkochB = [
...
const layeledB=['סיכון מועט','סיכון בינוני','סיכון מוגבר','הלכה יהודית']

const bituachB=['הראל פנסיה וגמל','כלל פנסיה וגמל',
    'מגדל מקפת קרנות פנסיה וקופות גמל','מנורה מבטחים פנסיה וגמל',
    'הפניקס פנסיה וגמל'
  ]
  const bateyhashkaotB=['אינפיניטי השתלמות, גמל ופנסיה','אלטשולר שחם גמל ופנסיה',
    'אנליסט קופות גמל','ילין לפידות ניהול קופות גמל','מור גמל ופנסיה'
    ,'מיטב גמל ופנסיה','סלייס גמל'
  ]


  async function bringNetunimB() {  
    fetchAllNetunimB();
    //maslulim();
  }
  async function fetchAllNetunimB(){
    await fetchkupotKlaliB();
    await fetchtsuaaHodshiB();
    await fetchNechasimB();
 }




async function fetchkupotKlaliB() {
  ...
  .then(xmlString => {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const mas = getMaslulType(shemkupa) ||  '';
        ...
        const sharp= row.getElementsByTagName("SHARP_RIBIT_HASRAT_SIKUN")[0]?.textContent ||  0;
          ...
          if (
            !excludedKupaNamesB.some(name => shemkupa.includes(name)) &&
            !tarsiyum &&
...
async function fetchtsuaaHodshiB() {
    
    fetch('bituachHodshi.xml')
